...
# Temp dir for assembling chunks
CHUNK_UPLOAD_DIR = Path(tempfile.gettempdir()) / 'unstressvn_chunks'


def _get_chunk_dir(upload_id):
    """Get temp directory for a specific upload session. Validates upload_id is a UUID."""
    try:
        uuid.UUID(upload_id)
...
def upload_chunk(request):
    ...
    POST /media-stream/admin/upload/chunk/

    FormData:
        upload_id:    UUID identifying this upload session
    ...
    try:
        ...
        return JsonResponse({'error': 'Invalid chunk_index or total_chunks'}, status=400)
    ...
    filename = request.POST.get('filename', 'unnamed')
...
def upload_complete(request):
    """
    Assemble chunks and upload to GDrive (or save locally).
    POST /media-stream/admin/upload/complete/

    FormData:
        upload_id:   UUID matching the chunks
        title:       media title
        media_type:  video|audio|podcast
        language:    language code
        level:       level code
        storage:     gdrive|local
        is_public:   true|false
    """
    upload_id = request.POST.get('upload_id', '')
    chunk_dir = _get_chunk_dir(upload_id)
    if not chunk_dir or not chunk_dir.exists():
        return JsonResponse({'error': 'Upload session not found'}, status=400)

    meta_path = chunk_dir / 'meta.json'
    if not meta_path.exists():
        return JsonResponse({'error': 'Upload metadata missing'}, status=400)
    ...
    if len(chunk_files) != total_chunks:
        return JsonResponse({
            'error': f'Missing chunks: received {len(chunk_files)}/{total_chunks}'
        }, status=400)
    ...
    try:
        with open(assembled_path, 'wb') as out:
            for cf in chunk_files:
                with open(cf, 'rb') as inp:
                    while True:
                        buf = inp.read(8 * 1024 * 1024)  # 8MB read buffer
                        if not buf:
                            break
                        out.write(buf)
        ...
        title = base_title or os.path.splitext(filename)[0].replace('-', ' ').replace('_', ' ').title()
        ...
        if use_gdrive:
            try:
                from .gdrive_upload import (
                    _get_gdrive_config, _build_drive_service,
                    upload_to_gdrive, get_folder_for_media_type
                )
                sa_dict, _ = _get_gdrive_config()
                if not sa_dict:
                    raise Exception('Google Drive chưa cấu hình')
                ...
                logger.error(f"Chunked GDrive upload failed: {e}", exc_info=True)
                errors.append(f'GDrive upload failed: {str(e)[:200]}')
                # Fallback to local
                from django.core.files.base import File
                with open(assembled_path, 'rb') as fh:
                    media.file.save(filename, File(fh), save=False)
                media.storage_type = 'local'
        else:
            from django.core.files.base import File
            with open(assembled_path, 'rb') as fh:
        ...
        logger.error(f"Chunked upload complete error: {e}", exc_info=True)
